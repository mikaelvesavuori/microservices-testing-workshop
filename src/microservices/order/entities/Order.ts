import { CustomerType, Market, Currency } from '../contracts/Order';
...
class Order {
  ...
  constructor(orderData: OrderDto) {
    ...
    this.customerType = orderData.customerType as CustomerType;
    this.market = orderData.market as Market;
  ...
  private checkIfMarketOffersB2b(orderMarket: Market) {
    const b2bMarkets = ['US'];
    return b2bMarkets.includes(orderMarket);
  }
  ...
  private validateOrderData() {
    ...
    if (this.customerType === 'B2B' && !this.checkIfMarketOffersB2b(this.market)) {
      throw new Error(
        `Unsupported market/customer type combo: "${this.market}" and "${this.customerType}"!`
      );
    }
  }
