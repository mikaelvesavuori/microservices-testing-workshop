...
import { emitEvent } from '../../../common/EmitEvent';
...
export async function CreateOrder(
  ...
  if (!event) return returnFail(400, 'Missing event!');
  ...
  try {
    ...
    /**
     * Handle payment use cases
     */
    handleUseCases(order);
    ...
    /**
     * Emit event to other systems, but only if this is not a test
     */
    ...
    console.error(error);
    return returnFail(400, error);
  }
}

function returnFail(statusCode: number, error: any) {
  return {
    statusCode,
    headers: {
      'Access-Control-Allow-Headers': 'Content-Type',
...
function handleUseCases(order: OrderB2B | OrderB2C): void {
  const { customerType, market } = order;
  /**
